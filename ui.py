...
class JarvisUI:
    ...
    def _show_normal_ui(self):
        ...
        LW = int(self.W * 0.72)
        LH = 138
        self.log_frame.place(x=(self.W - LW) // 2, y=self.H - LH - 36, width=LW, height=LH)
    ...
    def _draw_lock_screen(self):
        ...
        c.create_text(W // 2, 44, text="SECURITY PROTOCOL",
                      fill=C_RED, font=("Courier", 10, "bold"))
        c.create_text(W - 16, 31, text=time.strftime("%H:%M:%S"),
                      fill=C_PRI, font=("Courier", 14, "bold"), anchor="e")
        c.create_text(16, 31, text="🔒 LOCKED",
                      fill=C_RED, font=("Courier", 9), anchor="w")
        ...
        for i in range(3):
            r = cam_r + 15 + i * 8
            alpha = max(0, 80 - i * 25)
            c.create_oval(cam_cx - r, cam_cy - r, cam_cx + r, cam_cy + r,
                          outline=self._ac(0, 212, 255, alpha), width=2)
        ...
        if frame is not None:
            ...
            c.create_oval(cam_cx - cam_r, cam_cy - cam_r,
                          cam_cx + cam_r, cam_cy + cam_r,
                          fill="#001520", outline=C_DIM, width=2)
            c.create_text(cam_cx, cam_cy, text="📷 NO CAMERA",
                          fill=C_DIM, font=("Courier", 12, "bold"))
        ...
        c.create_line(cam_cx - cam_r + 10, scan_y_pos,
                      cam_cx + cam_r - 10, scan_y_pos,
                      fill=self._ac(0, 255, 255, 60), width=1)
        ...
        if self._lock_denied_flash > 0:
            ...
            c.create_rectangle(0, 0, W, H, fill=self._ac(255, 0, 0, flash_alpha), outline="")
            self._lock_denied_flash -= 1

        # Status box
        box_w = 320
        box_h = 50
        c.create_rectangle(W//2 - box_w//2, status_y - box_h//2,
                          W//2 + box_w//2, status_y + box_h//2,
                          fill="#00080d", outline=C_DIM, width=1)
        ...
        if conf > 0:
            ...
            bar_w = 200
            bar_h = 4
            c.create_rectangle(W//2 - bar_w//2, bar_y,
                              W//2 + bar_w//2, bar_y + bar_h,
                              fill="#001520", outline="")
            fill_w = int(bar_w * min(conf, 1.0))
            bar_color = C_GREEN if conf >= 0.85 else C_ACC2 if conf >= 0.7 else C_RED
            c.create_rectangle(W//2 - bar_w//2, bar_y,
                              W//2 - bar_w//2 + fill_w, bar_y + bar_h,
                              fill=bar_color, outline="")
            c.create_text(W//2 + bar_w//2 + 30, bar_y + 2,
                          text=f"{conf:.0%}", fill=C_DIM,
                          font=("Courier", 8), anchor="w")

        # Quality hint
        hint = state.get("quality_hint", "")
        if hint:
            c.create_text(W // 2, status_y + 75,
                          text=hint, fill=C_ACC,
                          font=("Courier", 9))
    ...
    def _draw_face_overlay(self, c, box, cam_cx, cam_cy, cam_r, state, frame):
        ...
        for bx, by, sdx, sdy in [(dx, dy, 1, 1),
                                   (dx + dw, dy, -1, 1),
                                   (dx, dy + dh, 1, -1),
                                   (dx + dw, dy + dh, -1, -1)]:
    ...
    def _enrollment_render_loop(self):
        ...
        c.create_text(W // 2, 22, text=SYSTEM_NAME,
                      fill=C_PRI, font=("Courier", 18, "bold"))
        c.create_text(W // 2, 44, text="FIRST-TIME SECURITY SETUP",
        ...
        if frame is not None:
            ...
            c.create_oval(cam_cx - cam_r, cam_cy - cam_r,
                          cam_cx + cam_r, cam_cy + cam_r,
                          fill="#001520", outline=C_DIM, width=2)
            c.create_text(cam_cx, cam_cy, text="Starting camera...",
                          fill=C_DIM, font=("Courier", 11))

        # Camera ring
        c.create_oval(cam_cx - cam_r - 3, cam_cy - cam_r - 3,
                      cam_cx + cam_r + 3, cam_cy + cam_r + 3,
                      outline=C_MID, width=2)

        # Instruction text
        instr_y = cam_cy + cam_r + 40
        c.create_text(W // 2, instr_y,
                      text=self._enroll_text,
                      fill=C_TEXT, font=("Courier", 11))
        ...
        if not self._enroll_started and not self._enroll_done:
            btn_y = instr_y + 60
            btn_w, btn_h = 200, 40
            c.create_rectangle(W//2 - btn_w//2, btn_y - btn_h//2,
                              W//2 + btn_w//2, btn_y + btn_h//2,
                              fill="#001520", outline=C_PRI, width=2,
                              tags="enroll_btn")
            c.create_text(W // 2, btn_y,
                          text="▸  BEGIN REGISTRATION",
                          fill=C_PRI, font=("Courier", 11, "bold"),
                          tags="enroll_btn")
        ...
        if self._enroll_done:
            done_y = instr_y + 40
            c.create_text(W // 2, done_y,
                          text="✅ REGISTRATION COMPLETE",
                          fill=C_GREEN, font=("Courier", 14, "bold"))
            c.create_text(W // 2, done_y + 25,
                          text="System will lock in 2 seconds...",
                          fill=C_DIM, font=("Courier", 9))

        # Tip text
        c.create_text(W // 2, H - 50,
                      text="Wrong camera? Edit config/face_config.py → CAMERA_INDEX",
                      fill=C_DIMMER, font=("Courier", 8))
    ...
    def _prompt_pin(self):
        ...
        tk.Label(top, text="Set 4-Digit Security PIN\n(Used for re-enrollment)",
                 fg=C_ACC, bg="#00080d", font=("Courier", 10)).pack(pady=10)

        pin_var = tk.StringVar()
        entry = tk.Entry(top, textvariable=pin_var, font=("Courier", 16),
                         show="*", justify="center", width=6)
        entry.pack(pady=10)
        entry.focus()
        ...
        def submit(event=None):
            ...
            if len(pin) >= 4 and pin.isdigit():
                ...
                pin_var.set("")
                tk.Label(top, text="Invalid PIN (4 digits min)", fg=C_RED,
                         bg="#00080d", font=("Courier", 8)).pack()

        top.bind("<Return>", submit)
        btn = tk.Button(top, text="SAVE", command=submit,
                        bg=C_ACC2, fg="white", font=("Courier", 10, "bold"))
        btn.pack(pady=5)
    ...
    def _draw(self):
        ...
        if self._has_face and face_pil is not None:  # pyre-ignore
            ...
            if (self._face_scale_cache is None or
                    abs(self._face_scale_cache[0] - self.scale) > 0.004):  # pyre-ignore
        ...
        mic_cx = W // 2
        if self.mic_active:
            c.create_oval(mic_cx - mic_r, mic_y - mic_r,
                          mic_cx + mic_r, mic_y + mic_r,
                          fill=C_PRI, outline=C_PRI, width=2, tags="mic_btn")
            c.create_rectangle(mic_cx - 4, mic_y - 9, mic_cx + 4, mic_y + 1,
                               outline="#000000", fill="#000000", width=1, tags="mic_btn")
            c.create_arc(mic_cx - 7, mic_y - 6, mic_cx + 7, mic_y + 6,
                         start=180, extent=180,
                         outline="#000000", width=2, style="arc", tags="mic_btn")
            c.create_line(mic_cx, mic_y + 6, mic_cx, mic_y + 10,
                          fill="#000000", width=2, tags="mic_btn")
            c.create_text(mic_cx, mic_y + mic_r + 12, text="MIC ON",
                          fill=C_PRI, font=("Courier", 9, "bold"), tags="mic_btn")
        else:
            c.create_oval(mic_cx - mic_r, mic_y - mic_r,
                          mic_cx + mic_r, mic_y + mic_r,
                          fill="", outline=C_RED, width=2, tags="mic_btn")
            c.create_rectangle(mic_cx - 4, mic_y - 9, mic_cx + 4, mic_y + 1,
                               outline=C_RED, fill="", width=1, tags="mic_btn")
            c.create_arc(mic_cx - 7, mic_y - 6, mic_cx + 7, mic_y + 6,
                         start=180, extent=180,
                         outline=C_RED, width=2, style="arc", tags="mic_btn")
            c.create_line(mic_cx, mic_y + 6, mic_cx, mic_y + 10,
                          fill=C_RED, width=2, tags="mic_btn")
            c.create_line(mic_cx - 8, mic_y + 10, mic_cx + 8, mic_y - 10,
                          fill=C_RED, width=2, tags="mic_btn")
            c.create_text(mic_cx, mic_y + mic_r + 12, text="MIC OFF",
                          fill=C_RED, font=("Courier", 9, "bold"), tags="mic_btn")
