...
Examples:
- "open camera" → {{"action": "open_camera", "value": null}}
- "kamera aç" → {{"action": "open_camera", "value": null}}
- "kapat kamera" → {{"action": "close_camera", "value": null}}
- "close camera" → {{"action": "close_camera", "value": null}}
...
- "write good morning on screen" → {{"action": "write_on_screen", "value": "good morning"}}  # noqa: E501
...
- If the user's intent is clear but uses different wording, map it to the closest action.  # noqa: E501
...
def computer_settings(
    ...
    parameters:
        action      : İşlem adı (verilmezse description'dan Gemini ile tespit edilir)  # noqa: E501
        description : Kullanıcının doğal dil komutu (herhangi bir dilde)
        value       : İşleme özgü değer (ses seviyesi, yazılacak metin, tekrar sayısı vb.)  # noqa: E501
    """
    if not _PYAUTOGUI:
    ...
    if action in ("open_camera", "camera_on", "start_camera"):
        if player and hasattr(player, "face_system") and player.face_system:
            if player.face_system.start_camera():
                if hasattr(player, "is_locked") and player.is_locked:
                    return "Camera activated. Scanning for your face, sir."
                return "Camera activated."
            else:
                return "Failed to activate camera."
        return "Camera unavailable."

    if action in ("close_camera", "camera_off", "stop_camera"):
        if player and hasattr(player, "face_system") and player.face_system:
            player.face_system.stop_camera()
            return "Camera deactivated, sir."
        return "Camera unavailable."

    if action in ("lock_screen", "lock", "lock_system"):
        lock_screen()
        if player and hasattr(player, "_lock"):
            player._lock("Voice command")
    ...
    if action in ("type_text", "write_on_screen", "type", "write"):
        ...
        return f"Typed: {text[:60]}"  # type: ignore
