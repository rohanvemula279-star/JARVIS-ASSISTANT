...
class TaskQueue:
    ...
    def submit(
        ...
        task = Task(
            priority=priority.value,  # type: ignore
        ...
        print(f"[TaskQueue] 📥 Task queued: [{task_id}] {goal[:60]}")  # type: ignore
        return task_id
    ...
    def _next_task(self) -> Task | None:
        ...
        for task in self._queue:
            if task.status == TaskStatus.PENDING and not task.cancel_flag.is_set():  # noqa: E501
                return task
        return None

    def _run_task(self, task: Task) -> None:
        print(f"[TaskQueue] ▶️ Running: [{task.task_id}] {task.goal[:60]}")  # type: ignore
        try:
